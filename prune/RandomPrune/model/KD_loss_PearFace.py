...
from models.metric import ArcFace, CosFace, PearFace
...
class KD_loss(nn.Module):
    def __init__(self, class_num, T=1, alpha=0.5, beta=1.0, gamma=1.0, embedding_size=512):
        ...
        self.pearface = PearFace(embedding_size=embedding_size, class_num=class_num).to(conf.device)
    ...
    def forward (self, teacher_outputs, student_outputs, labels):
        ...
        thetas = self.pearface(student_outputs, labels)
        cls_loss = self.criterion(thetas, labels)

        thetas_tea = self.pearface(teacher_outputs, labels)
        thetas_stu = self.pearface(student_outputs, labels)
