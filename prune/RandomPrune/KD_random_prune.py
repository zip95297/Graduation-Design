...
import time
from model.KD_loss_PearFace import KD_loss
...
crterion_KD=KD_loss(class_num=class_num,embedding_size=conf.embedding_size).to(device)
...
def train(epoch):
    ...
    for data, labels in dataloader :
        ...
        teacher_embeddings = teacher_model(data)

        loss_KD = crterion_KD(teacher_embeddings, embeddings, labels)


        thetas = metric_f(embeddings, labels)
        loss = criterion(thetas, labels)

        loss = loss + 15*loss_KD
