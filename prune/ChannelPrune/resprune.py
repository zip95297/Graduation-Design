...
from models import *
from test_diff_dataset import unique_image, group_image, featurize, test_in_train
from config import config as conf

from models.resnet18_with_cfg import ResNet18_with_config


# Prune settings
parser = argparse.ArgumentParser(description='PyTorch Slimming prune')
parser.add_argument('--dataset', type=str, default='casia-webface',
                    help='training dataset (default: casia-webface)')
...
def test(model,conf):
    acc,th=test_in_train(model, conf.test_list, conf.test_root, conf)
    print(f"acc:{acc},th:{th}")
...
newmodel = ResNet18_with_config(config=cfg)
if args.cuda:
    newmodel.cuda()
...
def copy_paramaters_to_new_model():
